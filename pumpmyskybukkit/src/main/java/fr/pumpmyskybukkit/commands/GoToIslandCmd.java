...
public class GoToIslandCmd implements ISubCommand{
	...
	public boolean onSubCommand(IslandCommandExecutor exec, Player sender, Command cmd, List<String> args) {
		...
		try {
			...
			sender.spigot().sendMessage(new ComponentBuilder(PlotManagerConstant.PLOT_CHAT_PREFIX + "§r§c Sans île, vous ne pouvez pas vous téléporter à §ccelle-ci !").create());
